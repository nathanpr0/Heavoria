...
// Admin and customer portals keep separate browser sessions. This lets a
// user status page stay available while an admin verifies the same order.
$portal = (($_GET['portal'] ?? 'user') === 'admin') ? 'admin' : 'user';
if (session_status() === PHP_SESSION_NONE) {
    session_name($portal === 'admin' ? 'HEAVORIA_ADMIN_SESSION' : 'HEAVORIA_USER_SESSION');
    session_start();
}
...
/**
 * Upgrade installations created with the earlier three-state ENUM status.
 * The current workflow has four explicit admin stages, so an ENUM from the
 * old schema would reject "Menunggu Verifikasi" with MySQL error 1265.
 */
function migrateOrderStatusColumn($pdo) {
    $statusType = strtolower(getColumnType($pdo, 'orders', 'status'));
    if (str_starts_with($statusType, 'enum(')) {
        $pdo->exec("ALTER TABLE orders MODIFY status VARCHAR(40) NOT NULL DEFAULT 'Menunggu Verifikasi'");
    }
    ...
    $pdo->exec("
        ...
        SET status = CASE status
            ...
            WHEN 'Diproses' THEN 'Dikonfirmasi'
            ELSE status
        END
    ");
}
...
/**
 * Replace the old demo menu IDs with the current canonical IDs. Updating the
 * order-item references first keeps existing transaction history intact.
 */
function removeLegacyDuplicateMenu($pdo) {
    $legacyToCanonical = [
        'menu-1' => 'sushi-roll',
        'menu-2' => 'nigiri',
        'menu-3' => 'towelcake-grape',
        'menu-4' => 'towelcake-mango',
        'menu-5' => 'towelcake-strawberry',
    ];

    $exists = $pdo->prepare('SELECT COUNT(*) FROM menu WHERE id = ?');
    $moveOrderItems = $pdo->prepare('UPDATE order_items SET menu_id = ? WHERE menu_id = ?');
    $deleteMenu = $pdo->prepare('DELETE FROM menu WHERE id = ?');

    foreach ($legacyToCanonical as $legacyId => $canonicalId) {
        $exists->execute([$legacyId]);
        $legacyExists = (int)$exists->fetchColumn() > 0;
        $exists->execute([$canonicalId]);
        $canonicalExists = (int)$exists->fetchColumn() > 0;

        if ($legacyExists && $canonicalExists) {
            $moveOrderItems->execute([$canonicalId, $legacyId]);
            $deleteMenu->execute([$legacyId]);
        }
    }
}
