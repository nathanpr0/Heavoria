...
switch ($action) {
    ...
    case 'update_status':
        ...
        try {
            ...
            $allowedTransitions = [
                'Menunggu Verifikasi' => ['Dikonfirmasi', 'Ditolak'],
            ...
            if ($requestedStatus === '') {
                $requestedStatus = $nextStatuses[0] ?? $currentStatus;
            }

            if ($requestedStatus !== $currentStatus && !in_array($requestedStatus, $nextStatuses, true)) {
                sendJsonResponse([
                    'success' => false,
                    'message' => 'Status pesanan tidak dapat dilompati atau diubah kembali.'
                ], 400);
            }
